...
{
    public sealed class MacOSDynamicWallpaperSetter : IDynamicWallpaperSetter, IDisposable
    {
        public string Platform => nameof(OSPlatform.OSX);

        private static readonly TimeSpan ActiveSpacePauseDuration = TimeSpan.FromSeconds(2);

        private readonly ILogger<MacOSDynamicWallpaperSetter> _logger;
            ...
            IWallpaperMonitorProvider monitorProvider)
        {
            ...
            {
                Interval = ActiveSpacePauseDuration
            };
            _visibilityTimer.Tick += OnVisibilityTimerTick;

            if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
            {
                _activeSpaceMonitor = new MacOSActiveSpaceMonitor();
                _activeSpaceMonitor.ActiveSpaceChanged += OnActiveSpaceChanged;
            }
        }
            ...
            CancellationToken token = default)
        {
            ...
            _logger.LogInformation(
                "macOS 动态壁纸已启动，共 {Count} 帧，显示器数={MonitorCount}，每个 NSScreen 独立窗口，耗时 {ElapsedMs}ms",
        ...
        private void OnActiveSpaceChanged()
        {
            Dispatcher.UIThread.Post(() =>
            {
                foreach (var window in _playbackWindows)
                {
                    window.PauseRenderingFor(ActiveSpacePauseDuration);
                }
        ...
        public void Dispose()
        {
            ...
            _visibilityTimer.Tick -= OnVisibilityTimerTick;
            if (_activeSpaceMonitor != null)
            {
                _activeSpaceMonitor.ActiveSpaceChanged -= OnActiveSpaceChanged;
                _activeSpaceMonitor.Dispose();
            }
        }
